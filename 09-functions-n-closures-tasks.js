...
function getPolynom() {
    ...
    return (x) => arr.map((element, index) => element*Math.pow(x, (arr.length-(++index)))).reduce((pValue, cValue)=> pValue+cValue);
}
 ...
 *
 */
const logger = (func, logFunc) => function() {
    const arr = [].slice.call(arguments);
